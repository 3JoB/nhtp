...
func (t *transferWriter) probeRequestBody() {
	...
	case rres := <-t.ByteReadCh:
		...
		if rres.n == 0 && rres.err == io.EOF {
			...
			if rres.err != nil {
				t.Body = io.MultiReader(&byteReader{b: rres.b}, errorReader{err: rres.err})
			} else {
				t.Body = io.MultiReader(&byteReader{b: rres.b}, t.Body)
			}
		} else if rres.err != nil {
			t.Body = errorReader{err: rres.err}
		}
	case <-timer.C:
...
func (t *transferReader) parseTransferEncoding() error {
	...
	// strict and simple.
	if len(raw) != 1 {
		return &unsupportedTEError{err: fmt.Sprintf("too many transfer encodings: %q", raw)}
	}
	if !ascii.EqualFold(raw[0], "chunked") {
		return &unsupportedTEError{err: fmt.Sprintf("unsupported transfer encoding: %q", raw[0])}
	}
...
var (
	singleCRLF = unsafeConvert.BytesReflect("\r\n")
	doubleCRLF = unsafeConvert.BytesReflect("\r\n\r\n")
)
...
func (b *body) Close() error {
	...
	case b.doEarlyClose:
		...
		if lr, ok := b.src.(*io.LimitedReader); ok && lr.N > maxPostHandlerReadBytes {
			...
			n, err = io.CopyN(io.Discard, bodyLocked{b: b}, maxPostHandlerReadBytes)
			if err == io.EOF {
				err = nil
			}
	...
	default:
		...
		// the trailer headers after the body, if present.
		_, err = io.Copy(io.Discard, bodyLocked{b: b})
	}
	b.closed = true
	return err
}
