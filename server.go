...
var (
	crlf       = unsafeConvert.BytesReflect("\r\n")
	colonSpace = unsafeConvert.BytesReflect(": ")
)
...
func (w *response) ReadFrom(src io.Reader) (n int64, err error) {
	...
	if !ok {
		return io.CopyBuffer(writerOnly{Writer: w}, src, buf)
	}
	...
	if !w.cw.wroteHeader {
		n0, err := io.CopyBuffer(writerOnly{Writer: w}, io.LimitReader(src, sniffLen), buf)
		n += n0
	...
	n0, err := io.CopyBuffer(writerOnly{Writer: w}, src, buf)
	n += n0
	return n, err
}
...
func (c *conn) readRequest(ctx context.Context) (w *response, err error) {
	...
	if !http1ServerSupportsRequest(req) {
		return nil, statusError{code: StatusHTTPVersionNotSupported, text: "unsupported protocol version"}
	}
...
var extraHeaderKeys = [][]byte{
	unsafeConvert.BytesReflect("Content-Type"),
	unsafeConvert.BytesReflect("Connection"),
	unsafeConvert.BytesReflect("Transfer-Encoding"),
}

var (
	headerContentLength = unsafeConvert.BytesReflect("Content-Length: ")
	headerDate          = unsafeConvert.BytesReflect("Date: ")
)
...
func badRequestError(e string) error { return statusError{code: StatusBadRequest, text: e} }
...
func (c *conn) serve(ctx context.Context) {
		...
		*c.tlsState = tlsConn.ConnectionState()
		if proto := c.tlsState.NegotiatedProtocol; validNextProto(proto) {
			if fn := c.server.TLSNextProto[proto]; fn != nil {
				h := initALPNRequest{ctx: ctx, c: tlsConn, h: serverHandler{srv: c.server}}
	...
	c.bufw = newBufioWriterSize(checkConnErrorWriter{c: c}, 4<<10)
	...
	for {
		...
		inFlightResponse = w
		serverHandler{srv: c.server}.ServeHTTP(w, w.req)
...
func Redirect(w ResponseWriter, r *Request, urls string, code int) {
	if u, err := url.Parse(urls); err == nil {
		...
		if u.Scheme == "" && u.Host == "" {
			...
			var query string
			if i := strings.Index(urls, "?"); i != -1 {
				urls, query = urls[:i], urls[i:]
			}

			// clean up but preserve trailing slash
			trailing := strings.HasSuffix(urls, "/")
			urls = path.Clean(urls)
			if trailing && !strings.HasSuffix(urls, "/") {
				urls += "/"
			}
			urls += query
		}
	...
	h.Set("Location", hexEscapeNonASCII(urls))
	...
	if !hadCT && r.Method == "GET" {
		body := "<a href=\"" + htmlEscape(urls) + "\">" + StatusText(code) + "</a>.\n"
		fmt.Fprintln(w, body)
	}
}
...
var silenceSemWarnContextKey = &contextKey{name: "silence-semicolons"}
...
func tlsRecordHeaderLooksLikeHTTP(hdr [5]byte) bool {
	switch unsafeConvert.StringReflect(hdr[:]) {
