...
func (t *Transport) roundTrip(req *Request) (*Response, error) {
	...
	origReq := req
	cancelKey := cancelKey{req: origReq}
...
func (t *Transport) CancelRequest(req *Request) {
	t.cancelRequest(cancelKey{req: req}, errRequestCanceled)
}
...
func (t *Transport) dialConn(ctx context.Context, cm connectMethod) (pconn *persistConn, err error) {
	...
	if s := pconn.tlsState; s != nil && s.NegotiatedProtocol != "" {
	...
	pconn.bw = bufio.NewWriterSize(persistConnWriter{pc: pconn}, t.writeBufferSize())
...
func (pc *persistConn) mapRoundTripError(req *transportRequest, startBytesWritten int64, err error) error {
	...
	if _, ok := err.(transportReadFromServerError); ok {
		if pc.nwrite == startBytesWritten {
			return nothingWrittenError{error: err}
		}
		// Don't decorate
		return err
	}
...
func (pc *persistConn) readLoop() {
	...
	for alive {
		...
		if err == nil {
			...
			err = transportReadFromServerError{err: err}
			closeErr = err
		}
...
func is408Message(buf []byte) bool {
	...
	if unsafeConvert.StringReflect(buf[:7]) != "HTTP/1." {
		return false
	}
	return unsafeConvert.StringReflect(buf[8:12]) == " 408"
}
...
func (pc *persistConn) writeLoop() {
	...
	for {
		...
		case wr := <-pc.writech:
			...
			if err != nil {
				if pc.nwrite == startBytesWritten {
					err = nothingWrittenError{error: err}
				}
			}
