...
func send(ireq *Request, rt RoundTripper, deadline time.Time) (resp *Response, didTimeout func() bool, err error) {
	...
	if err != nil {
		...
		if tlsErr, ok := err.(tls.RecordHeaderError); ok {
			...
			if unsafeConvert.StringReflect(tlsErr.RecordHeader[:]) == "HTTP/" {
...
// unclosed.
var ErrUseLastResponse error = &errs.Err{Op: "net/http", Err: "use last response"}
	...
	for {
		...
		if resp, didTimeout, err = c.send(req, deadline); err != nil {
			...
			if !deadline.IsZero() && didTimeout() {
				err = &httpError{
					err:     fmt.Sprintf("%v (Client.Timeout exceeded while awaiting headers)", err.Error()),
					timeout: true,
				}
			}
			return nil, uerr(err)
		...
		if c.Jar != nil && icookies != nil {
			...
			if changed {
				...
				for _, cs := range icookies {
					for _, c := range cs {
						ss = append(ss, fmt.Sprintf("%v=%v", c.Name, c.Value))
					}
				}
...
var clientRedirectErr error = &errs.Err{Op: "http", Err: "stopped after 10 redirects"}

func defaultCheckRedirect(req *Request, via []*Request) error {
	if len(via) >= 10 {
		return clientRedirectErr
	}
	return nil
}
...
func (b *cancelTimerBody) Read(p []byte) (n int, err error) {
	...
	if b.reqDidTimeout() {
		err = &httpError{
			err:     fmt.Sprintf("%v (Client.Timeout or context cancellation while reading body)", err.Error()),
			timeout: true,
		}
	}
	return n, err
