...
var http2errReadEmpty error = &errs.Err{Op: "http2", Err: "read from empty dataBuffer"}
...
var http2errFromPeer error = &errs.Err{Op: "http2", Err: "received from peer"}
...
var (
	http2errMixPseudoHeaderTypes error = &errs.Err{Op: "http2", Err: "mix of request and response pseudo headers"}
	http2errPseudoAfterRegular   error = &errs.Err{Op: "http2", Err: "pseudo header field after regular"}
)
...
var http2ErrFrameTooLarge error = &errs.Err{Op: "http2", Err: "frame too large"}
...
func http2parseDataFrame(fc *http2frameCache, fh http2FrameHeader, countError func(string), payload []byte) (http2Frame, error) {
	if fh.StreamID == 0 {
		...
		return nil, http2connError{Code: http2ErrCodeProtocol, Reason: "DATA frame with stream ID 0"}
	}
	f := fc.getDataFrame()
	...
	if int(padSize) > len(payload) {
		...
		return nil, http2connError{Code: http2ErrCodeProtocol, Reason: "pad size larger than data payload"}
	}
...
var (
	http2errStreamID    error = &errs.Err{Op: "http2", Err: "invalid stream ID"}
	http2errDepStreamID error = &errs.Err{Op: "http2", Err: "invalid dependent stream ID"}
	http2errPadLength   error = &errs.Err{Op: "http2", Err: "pad length too large"}
	http2errPadBytes    error = &errs.Err{Op: "http2", Err: "padding bytes must all be zeros unless AllowIllegalWrites is enabled"}
)
...
func http2parseUnknownFrame(_ *http2frameCache, fh http2FrameHeader, countError func(string), p []byte) (http2Frame, error) {
	return &http2UnknownFrame{http2FrameHeader: fh, p: p}, nil
}
...
func http2parseHeadersFrame(_ *http2frameCache, fh http2FrameHeader, countError func(string), p []byte) (_ http2Frame, err error) {
	...
	if fh.StreamID == 0 {
		...
		return nil, http2connError{Code: http2ErrCodeProtocol, Reason: "HEADERS frame with stream ID 0"}
	}
	var padLength uint8
...
func http2parsePriorityFrame(_ *http2frameCache, fh http2FrameHeader, countError func(string), payload []byte) (http2Frame, error) {
	if fh.StreamID == 0 {
		...
		return nil, http2connError{Code: http2ErrCodeProtocol, Reason: "PRIORITY frame with stream ID 0"}
	}
	if len(payload) != 5 {
		countError("frame_priority_bad_length")
		return nil, http2connError{Code: http2ErrCodeFrameSize, Reason: fmt.Sprintf("PRIORITY frame payload size was %d; want 5", len(payload))}
	}
...
func http2parseRSTStreamFrame(_ *http2frameCache, fh http2FrameHeader, countError func(string), p []byte) (http2Frame, error) {
	...
	return &http2RSTStreamFrame{http2FrameHeader: fh, ErrCode: http2ErrCode(binary.BigEndian.Uint32(p[:4]))}, nil
}
...
func http2parseContinuationFrame(_ *http2frameCache, fh http2FrameHeader, countError func(string), p []byte) (http2Frame, error) {
	if fh.StreamID == 0 {
		...
		return nil, http2connError{Code: http2ErrCodeProtocol, Reason: "CONTINUATION frame with stream ID 0"}
	}
	return &http2ContinuationFrame{http2FrameHeader: fh, headerFragBuf: p}, nil
}
...
func (fr *http2Framer) readMetaFrame(hf *http2HeadersFrame) (*http2MetaHeadersFrame, error) {
	...
	if invalid != nil {
		...
		return nil, http2StreamError{StreamID: mh.StreamID, Code: http2ErrCodeProtocol, Cause: invalid}
	}
	if err := mh.checkPseudos(); err != nil {
		...
		return nil, http2StreamError{StreamID: mh.StreamID, Code: http2ErrCodeProtocol, Cause: err}
	}
	return mh, nil
}
...
var http2goroutineSpace = unsafeConvert.BytesReflect("goroutine ")
...
var (
	http2clientPreface = unsafeConvert.BytesReflect(http2ClientPreface)
)
...
var http2errClosedPipeWrite error = &errs.Err{Op: "http2", Err: "write on closed buffer"}
...
var (
	http2errClientDisconnected error = &errs.Err{Op: "http2", Err: "client disconnected"}
	http2errClosedBody         error = &errs.Err{Op: "http2", Err: "body closed by handler"}
	http2errHandlerComplete    error = &errs.Err{Op: "http2", Err: "request body closed due to handler exiting"}
	http2errStreamClosed       error = &errs.Err{Op: "http2", Err: "stream closed"}
)

var http2responseWriterStatePool = sync.Pool{
	New: func() any {
		rws := &http2responseWriterState{}
		rws.bw = bufio.NewWriterSize(http2chunkWriter{rws: rws}, http2handlerChunkWriteSize)
		return rws
	},
}
...
func (sc *http2serverConn) rejectConn(err http2ErrCode, debug string) {
	...
	sc.framer.WriteGoAway(0, err, unsafeConvert.BytesReflect(debug))
	sc.bw.Flush()
	sc.conn.Close()
}
...
func (sc *http2serverConn) readFrames() {
	...
	for {
		...
		select {
		case sc.readFrameCh <- http2readFrameResult{f: f, err: err, readMore: gateDone}:
		case <-sc.doneServing:
			return
		}
		select {
...
func (sc *http2serverConn) serve() {
	...
	sc.writeFrame(http2FrameWriteRequest{
		write: http2writeSettings{
			{ID: http2SettingMaxFrameSize, Val: sc.srv.maxReadFrameSize()},
			{ID: http2SettingMaxConcurrentStreams, Val: sc.advMaxStreams},
			{ID: http2SettingMaxHeaderListSize, Val: sc.maxHeaderListSize()},
			{ID: http2SettingHeaderTableSize, Val: sc.srv.maxDecoderHeaderTableSize()},
			{ID: http2SettingInitialWindowSize, Val: uint32(sc.srv.initialStreamRecvWindowSize())},
		},
	})
	sc.unackedSettings++
...
var http2errPrefaceTimeout error = &errs.Err{Op: "http2", Err: "timeout waiting for client preface"}
...
func (sc *http2serverConn) writeDataFromHandler(stream *http2stream, data []byte, endStream bool) error {
	...
	*writeArg = http2writeData{streamID: stream.id, p: data, endStream: endStream}
...
var http2errHandlerPanicked error = &errs.Err{Op: "http2", Err: "handler panicked"}
...
func (sc *http2serverConn) processPing(f *http2PingFrame) error {
	...
	sc.writeFrame(http2FrameWriteRequest{write: http2writePingAck{pf: f}})
	return nil
}
...
func (sc *http2serverConn) newResponseWriter(st *http2stream, req *Request) *http2responseWriter {
	...
	*rws = http2responseWriterState{} // zero all the fields
	rws.conn = sc
	rws.bw = bwSave
	rws.bw.Reset(http2chunkWriter{rws: rws})
	rws.stream = st
	rws.req = req
	...
	defer func() {
		...
		if didPanic {
			...
			sc.writeFrameFromHandler(http2FrameWriteRequest{
				write:  http2handlerPanicRST{StreamID: rw.rws.stream.id},
...
func (sc *http2serverConn) write100ContinueHeaders(st *http2stream) {
	sc.writeFrameFromHandler(http2FrameWriteRequest{
		write:  http2write100ContinueHeadersFrame{streamID: st.id},
		stream: st,
	})
}
...
func (sc *http2serverConn) noteBodyReadFromHandler(st *http2stream, n int, err error) {
	...
	if n > 0 {
		select {
		case sc.bodyReadCh <- http2bodyReadMsg{st: st, n: n}:
		case <-sc.doneServing:
		}
	}
}
...
func (w *http2responseWriter) FlushError() error {
	...
	if rws.bw.Buffered() > 0 {
		...
		_, err = http2chunkWriter{rws: rws}.Write(nil)
		if err == nil {
			select {
			case <-rws.stream.cw:
...
// Push errors.
var (
	http2ErrRecursivePush    error = &errs.Err{Op: "http2", Err: "recursive push not allowed"}
	http2ErrPushLimitReached error = &errs.Err{Op: "http2", Err: "push would exceed peer's SETTINGS_MAX_CONCURRENT_STREAMS"}
)
...
var http2TEMayOnly error = &errs.Err{Op: "http2", Err: `request header "TE" may only be "trailers" in HTTP/2`}
...
const (
	...
	http2defaultUserAgent = "Go-nhtp-client/2.0"
...
func http2configureTransports(t1 *Transport) (*http2Transport, error) {
	...
	t2 := &http2Transport{
		ConnPool: http2noDialClientConnPool{http2clientConnPool: connPool},
		t1:       t1,
	}
	connPool.t = t2
	if err := http2registerHTTPSProtocol(t1, http2noDialH2RoundTripper{http2Transport: t2}); err != nil {
		return nil, err
	}
	...
	upgradeFn := func(authority string, c *tls.Conn) RoundTripper {
		...
		if used, err := connPool.addConnIfNeeded(addr, t2, c); err != nil {
			go c.Close()
			return http2erringRoundTripper{err: err}
		} else if !used {
...
var (
	http2errClientConnClosed    error = &errs.Err{Op: "http2", Err: "client conn is closed"}
	http2errClientConnUnusable  error = &errs.Err{Op: "http2", Err: "client conn not usable"}
	http2errClientConnGotGoAway error = &errs.Err{Op: "http2", Err: "Transport received Server's graceful shutdown GOAWAY"}
)
...
func (cc *http2ClientConn) setGoAway(f *http2GoAwayFrame) {
	...
	if cc.goAwayDebug == "" {
		cc.goAwayDebug = unsafeConvert.StringReflect(f.DebugData())
	}
...
var http2errRequestCanceled error = &errs.Err{Op: "net/http", Err: "request canceled"}
