...
func (h *Handler) ServeHTTP(rw http.ResponseWriter, req *http.Request) {
	...
	if len(req.TransferEncoding) > 0 && req.TransferEncoding[0] == "chunked" {
		...
		rw.Write(unsafeConvert.BytesReflect("Chunked request bodies are not supported by CGI."))
		return
	}
	...
	for {
		...
		headerLines++
		header, val, ok := strings.Cut(unsafeConvert.StringReflect(line), ":")
		if !ok {
			h.printf("cgi: bogus header line: %s", unsafeConvert.StringReflect(line))
			continue
		}
